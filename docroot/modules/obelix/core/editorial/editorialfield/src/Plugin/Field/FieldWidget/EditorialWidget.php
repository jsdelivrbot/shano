...
/**
 * @file
 * Contains \Drupal\editorialfield\Plugin\Field\FieldWidget\EditorialWidget.
 */
      ...
      if (isset($entities[$index]['entity_id'])) {
        ...
        switch ($mode) {
          ...
          case 'display_content_select_form':
            ...
            break;
          default:
            $entity_type_options = EditorialFieldConfigurationManager::getEntityOptions(TRUE);
...
//              '#dropbutton' => 'create',
  ...
  public function buildContentCreateElement(EntityInterface $entity, $delta, $region_name, $index, array &$form, FormStateInterface $form_state) {
    ...
    $wrapper_title = $this->t('Create element');
    ...
    if (!$entity->isNew()) {
      ...
      $wrapper_title = $this->t('Edit element');
    }

    return $this->buildOverlayWrapper($element, $wrapper_title);
  }
  ...
  public function buildContentSelectElement(EntityInterface $entity, $delta, $region_name, $index, array &$form, FormStateInterface $form_state) {
    ...
    return $this->buildOverlayWrapper($element, $this->t('Search element'));
  }
  ...
  public function buildItemSettingsForm(FieldItemListInterface $items, $delta, array $element, array &$form, FormStateInterface $form_state) {
    ...
    \Drupal::moduleHandler()
      ->alter('editorial_widget_layout_settings_form', $element['settings'], $form_state, $items[$delta]);
    ...
    return $this->buildOverlayWrapper($element, $this->t('Layout settings'));
  }
  ...
  public function buildItemRemoveForm(FieldItemListInterface $items, $delta, array $element, array &$form, FormStateInterface $form_state) {
    $element = parent::buildItemRemoveForm($items, $delta, $element, $form, $form_state); // TODO: Change the autogenerated stub
    return $this->buildOverlayWrapper($element, $this->t('Remove layout'));
  }
  ...
  public function buildContentRemoveElements(ContentEntityInterface $entity, $delta, $region_name, $index, array &$form, FormStateInterface $form_state) {
    $element = parent::buildContentRemoveElements($entity, $delta, $region_name, $index, $form, $form_state);
    return $this->buildOverlayWrapper($element, $this->t('Remove element'));
  }

  protected function buildOverlayWrapper($body, $title = NULL) {

    $prefix = '';
    $suffix = '';

    $prefix .= '<div class="editorial-overlay-wrapper">';
    $prefix .= '<div class="editorial-overlay-content">';

    if ($title) {
      $prefix .= '<h3 class="editorial-overlay-title">' . $title .'</h3>';
    }
