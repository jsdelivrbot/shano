...
/**
 * @file
 * Contains \Drupal\editorial\Plugin\Field\FieldWidget\EditorialWidgetBase.
 */
...
use Drupal\editorial\Plugin\Field\FieldType\EditorialField;
...
//      $field_state['items'] = self::filterItemValues($field_state['items']);
  ...
  public function getLayoutOptions($only_available = FALSE) {
    $options = EditorialFieldGlobalConfigurationManager::getLayoutOptions(TRUE);
  ...
  public function getEntityOptions($only_available = FALSE) {
    $options = EditorialFieldGlobalConfigurationManager::getEntityOptions(TRUE);
    $settings = $this->getSettings();
  ...
  public function getEntityBundleOptions($entity_type_ids, $only_available = FALSE) {
    $entity_type_options = EditorialFieldGlobalConfigurationManager::getEntityOptions($only_available);
    $settings = $this->getSettings();
    $options = [];

    $entity_type_ids = (array) $entity_type_ids;
    foreach ($entity_type_ids as $entity_type_id) {
      $entity_bundles = EditorialFieldGlobalConfigurationManager::getEntityBundleOptions($entity_type_id);
